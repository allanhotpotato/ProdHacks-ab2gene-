import { RAG_SYSTEM_INSTRUCTION } from '../config.js';
import { generateModelText } from './llm.js';

export function isNonAutofillField(fieldKey: string, inputType: string, tagName: string): boolean {
...
export async function generateAnswerForQuestion(
  ...
  const instructions =
    RAG_SYSTEM_INSTRUCTION + (wordLimit ? ` Keep your answer within ${wordLimit} words.` : '');
