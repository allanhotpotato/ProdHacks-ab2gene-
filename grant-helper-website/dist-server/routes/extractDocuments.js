...
import { CHUNK_CHAR_SIZE, CHUNK_OVERLAP, createUserSupabaseClient, persistChunksAndEmbeddings, splitTextIntoChunks, } from '../services/rag.js';
const router = Router();
...
router.post('/', upload.array('files', 20), async (req, res) => {
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const canPersist = Boolean(accessToken && docId && trimmed && documentIds?.length === files.length);
            if (!canPersist) {
                continue;
            }
