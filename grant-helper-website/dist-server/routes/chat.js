...
router.post('/', async (req, res) => {
    try {
        const { grantContext, messages, accessToken: bodyToken } = req.body;
        ...
        const text = await generateModelText(buildSystemInstruction(grantContext, retrievedChunks || undefined), toSend, priorHistory);
        res.json({ reply: text ?? '' });
    }
    catch (err) {
