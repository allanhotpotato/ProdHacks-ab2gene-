...
const RAG_EMBEDDING_MODEL = process.env.RAG_EMBEDDING_MODEL?.trim() || 'onnx-community/all-MiniLM-L6-v2-ONNX';
...
const RAG_EMBEDDINGS_DISABLED = process.env.RAG_EMBEDDINGS_DISABLED === '1' || /^true$/i.test(process.env.RAG_EMBEDDINGS_DISABLED ?? '');
...
if (!OPENAI_API_KEY && !GEMINI_API_KEY) {
    console.error('Missing API key. Add OPENAI_API_KEY=your_key or GEMINI_API_KEY=your_key to grant-helper-website/.env.');
    process.exit(1);
}
...
`;
function getFieldSpecificGuidance(fieldKey) {
    switch (fieldKey) {
        case 'contact_name':
...
function buildSystemInstruction(profileContext, grantContext) {
...
function parseEmbeddingColumn(value) {
...
async function getRagChatContext(q, accessToken) {
...
async function fetchUserDocumentContext(userId) {
...
const RAG_EMBEDDING_BATCH_SIZE = Math.max(1, Math.min(32, Number(process.env.RAG_EMBEDDING_BATCH_SIZE) || 8));
...
async function embedTextsWithTransformers(texts) {
...
async function persistChunksAndEmbeddings(userClient, userId, documentId, filename, chunks, embeddings) {
    const batchSize = 50;
...
async function resolveOrganizationContext(organizationProfile, userId) {
...
async function generateAnswerForQuestion(context, question, wordLimit) {
...
async function structureOrganizationProfile(context) {
    const emptyProfile = Object.fromEntries(STRUCTURED_PROFILE_KEYS.map((key) => [key, '']));
...
function extractStructuredProfileHeuristics(context) {
    const profile = Object.fromEntries(STRUCTURED_PROFILE_KEYS.map((key) => [key, '']));
...
async function generateGeminiText(instructions, prompt) {
    if (!genAI) {
...
async function generateModelText(instructions, prompt, history = []) {
    if (OPENAI_API_KEY) {
        const messages = [
            ...history.map((message) => ({
...
async function extractOrganizationProfile(documentText) {
...
async function scoreGrantMatch(orgProfile, grantData) {
...
async function quickEligibilityCheck(orgProfile, grantData) {
...
async function generateApplicationTips(orgProfile, grantData, matchScore) {
 ...
 */
app.post('/api/extract-documents', upload.array('files', 20), async (req, res) => {
    try {
...
app.post('/api/chat', async (req, res) => {
    try {
        const { grantContext, profileContext, messages } = req.body;
        ...
        const toSend = lastMessage?.role === 'user'
            ? lastMessage.content
        ...
        const text = await generateModelText(buildSystemInstruction(profile, grantContext), toSend, priorHistory);
        res.json({ reply: text ?? '' });
    }
    catch (err) {
...
app.post('/api/autofill-field', async (req, res) => {
    try {
        const { questionText, fieldKey = '', descriptor = '', tagName = '', inputType = '', pageTitle = '', pageUrl = '', organizationProfile = '', grantContext = '', userId = '', } = req.body;
...
app.post('/api/autofill-fields', async (req, res) => {
    try {
        const { fields = [], pageTitle = '', pageUrl = '', organizationProfile = '', grantContext = '', userId = '', } = req.body;
...
app.post('/api/profile-structure', async (req, res) => {
    try {
        const { organizationProfile = '', userId = '' } = req.body;
...
app.post('/api/google-form/prefill', async (req, res) => {
    try {
 ...
 */
app.get('/api/google-form/prefill-url', (req, res) => {
...
app.post('/api/grants/smart-match', async (req, res) => {
    try {
...
function normalizeCatalogGrant(g) {
    return {
...
app.get('/api/grants/catalog', (req, res) => {
    try {
 ...
 */
app.post('/api/ein-lookup', async (req, res) => {
    try {
